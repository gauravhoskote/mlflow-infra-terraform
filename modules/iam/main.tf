...
resource "aws_iam_instance_profile" "ecs_instance_profile" { name = "${var.name}-ecsInstanceProfile" role = aws_iam_role.ecs_instance_role.name }

resource "aws_iam_role" "batch_service_role" {
  name = "${var.name}-batch-service-role"
  assume_role_policy = jsonencode({ Version="2012-10-17", Statement=[{Effect="Allow", Principal={Service="batch.amazonaws.com"}, Action="sts:AssumeRole"}] })
}
